...
class DMTETSynthesisNetwork(torch.nn.Module):
    ...
    def get_sdf_deformation_prediction(
        ...
        :param ws: latent code for the geometry # [1, 2, 512]
        :param position: the location of tetrahedron vertices
        :param sdf_feature: triplane feature map for the geometry # [1, 96, 256, 256]
        :return:
        '''
        if position is None:
    ...
    def get_geometry_prediction(self, ws, sdf_feature=None):
        ...
        for i_batch in range(n_batch):
            verts, faces = self.dmtet_geometry.get_mesh(v_deformed[i_batch], sdf[i_batch].squeeze(dim=-1), with_uv=False, indices=tets)
            v_list.append(verts)
            f_list.append(faces)
    ...
    def extract_3d_shape(
        ...
        if self.one_3d_generator:
            sdf_feature, tex_feature = self.generator.get_feature(
                ws[:, :self.generator.tri_plane_synthesis.num_ws_tex], # 7
                ws_geo[:, :self.generator.tri_plane_synthesis.num_ws_geo] # 20
            ) # [1, 96, 256, 256] x 2
            ws = ws[:, self.generator.tri_plane_synthesis.num_ws_tex:] # [1, 2, 512]
            ws_geo = ws_geo[:, self.generator.tri_plane_synthesis.num_ws_geo:] # [1, 2, 512]
        ...
        if self.dmtet_geometry.renderer.ctx is None:
            # kiui: CUDA context is necessary to run both dr and gui on linux
            import os
            if os.name == 'nt':
        ...
        for v, f in zip(mesh_v, mesh_f):
            uvs, mesh_tex_idx, gb_pos, mask = xatlas_uvmap(self.dmtet_geometry.renderer.ctx, v, f, resolution=texture_resolution)
            all_uvs.append(uvs)
...
class GeneratorDMTETMesh(torch.nn.Module):
    ...
    def generate_3d_mesh(
        ...
        if use_mapping:
            ws = self.mapping(tex_z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
            ws_geo = self.mapping_geo(geo_z, c, truncation_psi=truncation_psi, truncation_cutoff=truncation_cutoff, update_emas=update_emas)
        else:
            ws = tex_z
            ws_geo = geo_z
